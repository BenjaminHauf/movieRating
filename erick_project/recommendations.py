...
def generate_recommendations(user, reco_num_gpt):
    ...
    """
    ##----- With this part the code retrun a string output not a list with 1 value and also informa if there some error when try to connect to chat GPT ----------###############
    response = ''
    try:
        for i in get_chatgpt_response(text_input, reco_num_gpt, 1):
            response += i
    except Exception as e:
        response = 'Failed to communicate with Chat GPT: \n{}'.format(str(e))
